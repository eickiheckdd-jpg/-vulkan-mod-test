...
public class VulkanParticleRenderer {
    private static final int MAX_PARTICLES = 8192;
    private static final int MAX_VERTICES_PER_PARTICLE = 4;
    private static final int VERTEX_SIZE = 16; // pos(12) + color(4) = 16
    ...
    private static final Deque<Particle> particleQueue = new ArrayDeque<>();
    ...
    public static synchronized void initialize() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            initialized = true;
            VulkanMod.LOGGER.info("Particle renderer initialized");
        } catch (Exception e) {
            VulkanMod.LOGGER.error("Failed to initialize particle renderer: {}", e.getMessage());
            e.printStackTrace();
        }
    }

    private static void createVertexBuffer(MemoryStack stack) {
        long bufferSize = (long) MAX_PARTICLES * MAX_VERTICES_PER_PARTICLE * VERTEX_SIZE;
        ...
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to create particle vertex buffer: " + result);
        }
        ...
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to allocate particle vertex buffer memory: " + result);
        }
        vertexBufferMemory = pBufferMemory.get(0);

        result = vkBindBufferMemory(VulkanDevice.getDevice(), vertexBuffer, vertexBufferMemory, 0);
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to bind particle vertex buffer memory: " + result);
        }
    }
    ...
    private static void createStagingBuffer(MemoryStack stack) {
        ...
        int result = vkCreateBuffer(VulkanDevice.getDevice(), bufferInfo, null, pBuffer);
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to create particle staging buffer: " + result);
        }
        ...
        result = vkAllocateMemory(VulkanDevice.getDevice(), allocInfo, null, pBufferMemory);
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to allocate particle staging buffer memory: " + result);
        }
        ...
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to bind particle staging buffer memory: " + result);
        }
    }
    ...
    private static void createPipeline(MemoryStack stack) {
        ...
        if (result != VK_SUCCESS) {
            throw new RuntimeException("Failed to create particle pipeline layout: " + result);
        }
    ...
    public static void addParticle(float x, float y, float z,
                                    float u, float v,
        ...
        particle.size = size;
        particle.life = life;
        particleQueue.addLast(particle);

        if (particleQueue.size() > MAX_PARTICLES) {
            particleQueue.pollFirst();
        }
    }
    ...
    public static void uploadAndRender(long commandBuffer) {
        ...
        try (MemoryStack stack = stackPush()) {
            int particleCount = Math.min(particleQueue.size(), MAX_PARTICLES);
            long totalSize = (long) particleCount * MAX_VERTICES_PER_PARTICLE * VERTEX_SIZE;

            FloatBuffer vertexData = stack.floats(particleCount * MAX_VERTICES_PER_PARTICLE * VERTEX_SIZE / 4);
            int vertexIndex = 0;
            ...
            for (Particle particle : particleQueue) {
                ...
                for (int i = 0; i < 4; i++) {
                    vertexData.put(positions[i * 3]);
                    vertexData.put(positions[i * 3 + 1]);
                    vertexData.put(positions[i * 3 + 2]);
                    vertexData.put(uvs[i * 2]);
                    vertexData.put(uvs[i * 2 + 1]);
                    vertexData.put(particle.r);
                    vertexData.put(particle.g);
                    vertexData.put(particle.b);
                    vertexData.put(particle.a);
                    vertexIndex++;
                }
            }
            vertexData.flip();
            ...
            if (result == VK_SUCCESS) {
                ...
                MemoryUtil.memCopy(MemoryUtil.memAddress(vertexData), dstPtr, totalSize);
